...
- 初期実装はHMAC署名した短期セッションを `HttpOnly` Cookieで保持し、サーバー側で署名と有効期限を検証する
- セッションのアイドルタイムアウトは10分とし、管理画面上の操作が続く場合のみ有効期限を更新する
- Phase 1は管理者1名固定とし、メールアドレスとscryptハッシュ済みパスワードをサーバー側環境変数から取得する。`admins` テーブルは作成しない
- 複数管理者や権限管理が必要になった場合は、管理者データ取得処理を `admins` テーブル参照へ差し替える
...
| `ADMIN_EMAIL` | Phase 1管理者のログインID | 非公開 |
| `ADMIN_PASSWORD_HASH` | `scrypt:<salt>:<hash>` 形式の管理者パスワードハッシュ | 非公開 |
